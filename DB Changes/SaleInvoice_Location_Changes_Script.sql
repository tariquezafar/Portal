...
ALTER proc [dbo].[proc_AddEditSaleInvoice]              
(             
@InvoiceId bigint,             
@InvoiceDate date,   
...
RtoRegsIGST_Amt,RtoRegsCGST_Perc,RtoRegsSGST_Perc,RtoRegsIGST_Perc,VehicleInsuranceValue,EwayBillNo,SaleEmpId,
SaleInvoiceType,LocationId) 
...
@RtoRegsCGST_Perc,@RtoRegsSGST_Perc,@RtoRegsIGST_Perc,@VehicleInsuranceValue,@EwayBillNo,@SaleEmpId, @SaleInvoiceType,@LocationId)              
...
GO
-- proc_GetSaleInvoices '','','','2021-04-01','1','2022-03-31',1,'0','Popup','Final','','0','',0
  
ALTER proc [dbo].[proc_GetSaleInvoices]
(                      
...
isnull(E.ConsigneeName,'''') ConsigneeName ,  
isnull(CN.CustomerCode,'''') ConsigneeCode ,  
E.SONo,             
E.City,
...
isnull(E.BasicValue-((dbo.udf_GetTotalSICGSTAmount(E.InvoiceId)+dbo.udf_GetTotalSISGSTAmount(E.InvoiceId)+dbo.udf_GetTotalSIIGSTAmount(E.InvoiceId))-dbo.udf_GetTotalSIDiscountAmount(E.InvoiceId)),0) as BasicAmt,                  
isnull(E.BasicValue,0) BasicValue,
...
left join [Location] LC ON ISNULL(LC.LocationId,0)=ISNULL(E.LocationId,0)
where  E.CompanyId='+ cast(@CompanyId as varchar) + ' ';                      
set @strSql=@strSql + ' and  E.InvoiceDate BETWEEN '''+ cast(@FromDate as varchar) + ''' AND  '''+ cast(@ToDate as varchar) + ''' ';                      
...
if @CustomerCode<>''                      
begin                      
set @strSql=@strSql + ' and  c.CustomerCode =''' + @CustomerCode + ''' ';                      
end                      
...
if @InvoiceNo<>''                      
begin                      
set @strSql=@strSql + ' and E.InvoiceNo like ''%' + @InvoiceNo + '%''';                      
end              
if @RefNo<>''                      
begin                      
set @strSql=@strSql + ' and E.RefNo like ''%' + @RefNo + '%''';                      
end          
if @InvoiceType<>'0' and @InvoiceType<>''            
begin                      
set @strSql=@strSql + ' and E.InvoiceType =''' + @InvoiceType + '''';                      
end              
if @DisplayType<>''          
begin                          
 set @strSql=@strSql + ' and E.InvoiceId not in (Select InvoiceId from DeliveryChallan) ';                          
end       
...
if @SaleType<>'0'                    
begin                      
set @strSql=@strSql + ' and E.SaleType =''' + @SaleType + '''';                      
end          
...
if @SaleInvoiceType='Service'                    
begin                      
set @strSql=@strSql + ' and E.SaleInvoiceType =''' + @SaleInvoiceType + '''';                       
end 
...
begin
set @strSql=@strSql + ' and E.SaleInvoiceType =''Sale'''; 
end
                        
set @strSql=@strSql + ' order by E.InvoiceDate Desc, E.InvoiceNo Desc ';         
print @strSql
...
ALTER proc [dbo].[proc_GetSaleInvoiceDetail] --proc_GetSaleInvoiceDetail 8           
(                  
@InvoiceId int                
)                  
as                  
