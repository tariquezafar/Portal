$(document).ready(function () {
  ...
  .autocomplete("instance")._renderItem = function (ul, item) {
    ...
    if (hdnPaymentTrnId.val() != "" && hdnPaymentTrnId.val() != "0" && hdnAccessMode.val() != "" && hdnAccessMode.val() != "0") {
        setTimeout(
       function () {
           GetCustomerPaymentDetail(hdnPaymentTrnId.val());
       }, 2000);
...
function GetCustomerPaymentDetail(paymenttrnId) {
    $.ajax({
        type: "GET",
        asnc: false,
        url: "../CustomerPayment/GetCustomerPaymentDetail",
        ...
        success: function (data) { 
            ...
            $("#btnAddNew").show();
            $("#btnPrint").show();
            if (data.CustomerPayment_Status == true) {
...
function SaveData() {
    ...
    var customerpaymentViewModel = {
        ...
        BookId: ddlBookType.val(),
        Amount: txtAmount.val(),
        ValueDate: txtValueDate.val(),
        Remarks: txtRemarks.val(),
        CustomerPayment_Status: chkstatus
    };
    ...
    var requestData = { customerpaymentViewModel: customerpaymentViewModel };
    $.ajax({
        url: "../CustomerPayment/AddEditCustomerPayment?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../CustomerPayment/AddEditCustomerPayment?PaymentTrnId=" + data.trnId + "&AccessMode=2";
                   }, 2000);
