...
function CalculateGrossandNetValues() {
    ...
    $("#txtTotalValue").val(parseFloat(
        parseFloat(basicValue)
        + parseFloat(taxValue)
        + parseFloat(freightValue)
        + parseFloat(loadingValue)
        + parseFloat(insuranceValue)
        + parseFloat(freightCGST)
        + parseFloat(freightSGST)
        + parseFloat(freightIGST)
        + parseFloat(loadingCGST)
        + parseFloat(loadingSGST)
        + parseFloat(loadingIGST)
        + parseFloat(insuranceCGST)
        + parseFloat(insuranceSGST)
        + parseFloat(insuranceIGST)
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.ReverseChargeApplicable == true)
            {
                ...
                $("#txtReverseChargeAmount").val(data.RevserseChargeAmount);
            }
...
function SaveData() {
    ...
    var chkReverseCharge = $("#chkReverseChargeApplicable").is(':checked') ? true : false;

    var txtRevserseAmount = $("#txtReverseChargeAmount");
    ...
    if (txtIdTypeName.val() != null || txtIdTypeName.val()=='')
    {
        IdTypeName1 = txtIdTypeName.val();
    }
    if (txtIDTypeValue.val() != null || txtIDTypeValue.val() !='') {
        IDTypeValue1 = txtIDTypeValue.val();
    }
    if (txtAdharcardNo.val() != null || txtAdharcardNo.val() !='') {
        AdharcardNo1 = txtAdharcardNo.val();
    }
    if (txtPancard.val() != null || txtPancard.val() !='') {
        Pancard1 = txtPancard.val();
    }
    ...
    if (txtBContactPerson.val().trim() == "") {
        ShowModel("Alert", "Please Enter Party billing Contact Person Name")
        txtBContactPerson.focus();
        return false;
    }
    ...
    if ($("#chkReverseCharge").is(':checked') && txtRevserseAmount.val() == "")
    {
    ...
    if (txtRTORegIGST_Amt.val() == "" || parseFloat(txtRTORegIGST_Amt.val()) == 0) {
        rTORegIGSTPerc = 0;
    }
    else {
    ...
    if (txtRTORegCGST_Amt.val() == "" || parseFloat(txtRTORegCGST_Amt.val()) == 0) {
        rTORegCGSTPerc = 0;
    }
    else {
    ...
    var rTOSGSTPerc = 0;
    if (txtRTORegSGST_Amt.val() == "" || parseFloat(txtRTORegSGST_Amt.val()) == 0) {
        rTOSGSTPerc = 0;
    }
    else {
    ...
    var soViewModel = {
        ...
        ReverseChargeApplicable: chkReverseCharge,
        RevserseChargeAmount: txtRevserseAmount.val(),
        ...
        RtoRegsValue: txtRTOReg.val(),
        RtoRegsCGST_Amt: txtRTORegCGST_Amt.val(),
        RtoRegsSGST_Amt: txtRTORegSGST_Amt.val(),
        RtoRegsIGST_Amt: txtRTORegIGST_Amt.val(),
        RtoRegsCGST_Perc: rTORegCGSTPerc,
        RtoRegsSGST_Perc: rTOSGSTPerc,
        RtoRegsIGST_Perc: rTORegIGSTPerc,
        VehicleInsuranceValue: txtVehicleInsurance.val(),
    ...
    $('#tblDocumentList tr').each(function (i, row) {
        var $row = $(row);
        var sODocId = $row.find("#hdnSODocId").val();
        var documentSequenceNo = $row.find("#hdnDocumentSequenceNo").val();
        var documentTypeId = $row.find("#hdnDocumentTypeId").val();
        var documentTypeDesc = $row.find("#hdnDocumentTypeDesc").val();
        var documentName = $row.find("#hdnDocumentName").val();
    ...
    if (hdnSOId.val() != null && hdnSOId.val() != 0) {
        accessMode = 2;//Edit Mode
    }

    var requestData = { soViewModel: soViewModel, soProducts: soProductList, soTaxes: soTaxList, soTerms: soTermList, soDocuments: soDocumentList };
    $.ajax({
        url: "../SO/AddEditSO?AccessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../SO/AddEditSO?SOId=" + data.trnId + "&AccessMode=3";
                   }, 3000);
...
function SearchQuotation() {
    ...
    var requestData = { quotationNo: txtQuotationNo.val().trim(), customerName: txtCustomerName.val().trim(), refNo: txtRefNo.val().trim(), fromDate: txtFromDate.val(), toDate: txtToDate.val(), displayType: "Popup", approvalStatus: "Final", companyBranchId: ddlCompanyBranch.val() };
    $.ajax({
...
function SelectQuotation(quotationId, quotationNo, quotationDate, customerId, customerCode, customerName, companyBranchId) {
    ...
    $("#ddlBCustomerBranch").attr('disabled', true);
    $("#txtBContactPerson").attr('readonly', true);
    $("#txtBAddress").attr('readonly', true);
    $("#txtBCity").attr('readonly', true);

    $("#ddlSCustomerBranch").attr('disabled', true);
    $("#txtSContactPerson").attr('readonly', true);
    $("#txtSAddress").attr('readonly', true);
    $("#txtSCity").attr('readonly', true);
...
function CopyCurrentAddress() {
    if ($("#chkSamePermanentAddress").is(':checked')) {
        ...
        $("#ddlSCustomerBranch").val($("#ddlBCustomerBranch").val());
    ...
    $('#tblProductList tr').each(function (i, row) {
        ...
        if (hdnProductId != undefined) {
            ...
            if (totalPrice != undefined) {
                ...
                $row.find("#tdTotalPrice").text(itemTotal);
...
function changeReverseChargeStatus() {
    if ($("#chkReverseCharge").is(':checked')) {
        ...
        $("#txtRevserseAmount").val(parseFloat(TotalGST).toFixed(2));


        $("#txtRevserseAmount").attr("disabled", true);
    }
    else {
        $("#txtRevserseAmount").attr("disabled", true);
        $("#txtRevserseAmount").val("");
    }
}
