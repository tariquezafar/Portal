$(document).ready(function () {
    ...
    $("#ddlCurrency").attr('readOnly', true);
    $("#txtRefNo").attr('readOnly', true);
    $("#txtBAddress").attr('readOnly', true);
    $("#txtBCity").attr('readOnly', true);
    $("#ddlSCustomerBranch").attr('readOnly', true);
    $("#txtShippingAddress").attr('readOnly', true);
    $("#txtShippingAddress").attr('readOnly', true);
...
.autocomplete("instance")._renderItem = function (ul, item) {
    ...
    $("#txtPODate,#txtRefDate,#txtDeliveryDate,#txtIndentFromDate,#txtIndentToDate,#txtQuotationFromDate,#txtQuotationToDate").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd-M-yy',
        yearRange: '-10:+100',
...
function GetPOProductList(poProducts) {
    ...
    $.ajax({
        url: "../PO/GetPOProductApprovalList",
        cache: false,
...
function GetPODetail(poId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            {
                $("#ddlPOApprovel").val(data.ApprovalStatus)
                $("#btnUpdate").hide();
                $("#btnSave").hide();
            }
            ...
            if (data.CancelByUserName != "") {
                $("#divModified").show();
                $("#txtModifiedBy").val(data.CancelByUserName);
                $("#txtModifiedDate").val(data.CancelDate);
                $("#txtRejectedReason").val(data.CancelReason);
...
function SaveData() {
    ...
    var hdnPOId = $("#hdnPOId");
    var ddlPOApprovel = $("#ddlPOApprovel");
    var txtRejectedReason = $("#txtRejectedReason");

    if (ddlPOApprovel.val() == "0" || ddlPOApprovel.val() == undefined) {
        ShowModel("Alert", "Please select PO Approvel Status")
        return false;

    }
    if (ddlPOApprovel.val() == "Rejected") {
        if (txtRejectedReason.val() == "") {
            ShowModel("Alert", "Please Enter PO Reject Reason")
            return false;
    ...
    var pOViewModel = {
        ...
        ApprovalStatus: ddlPOApprovel.val()
    };

    var requestData = { pOViewModel: pOViewModel };
    $.ajax({
        url: "../PO/ApprovalPO",
        cache: false,
        type: "POST",
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../PO/ApprovalPO?poId=" + $("#hdnPOId").val() + "&AccessMode=3";
                   }, 1000);
...
function SearchPurchaseQuotation() {
    ...
    var requestData = { quotationNo: txtQuotationNo.val().trim(), vendorName: txtVendorName.val(), fromDate: txtFromDate.val(), toDate: txtToDate.val(), displayType: "Popup", approvalStatus: "Final" };
    $.ajax({
...
function SelectQuotation(quotationId, quotationNo, quotationDate, requisitionId, requisitionNo, requisitionDate) {
...
function EnableDisableRejectReason() {
    var approvalStatus = $("#ddlPOApprovel option:selected").text();
