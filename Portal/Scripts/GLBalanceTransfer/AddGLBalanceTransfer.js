...
function GetFinYearDetail() {
    $.ajax({
        type: "GET",
        asnc: false,
        url: "../GLBalanceTransfer/GetFinYearDetail",
        data: { },
        dataType: "json",
...
function GetGLBalanceTransfer() {   
    ...
    $.ajax({
        url: "../GLBalanceTransfer/GetGLBalanceTransfer",
        data: requestData,
        dataType: "html",
        type: "GET",
        error: function (err) {
...
function SaveData() {
    if (confirm("Are you sure to Transfer GL Closing Balance.")) {
        var hdnTransferId = $("#hdnGLTransferId");
        var hdnPreviousFinYearId = $("#hdnPreviousFinYearId");
        ...
        $('#tblFinYearProductList tr').each(function (i, row) {
            ...
            if (gLId != undefined) {
                ...
                gLDetailViewModelList.push(gLDetailOpening);
            }
        });
        ...
        var requestData = { gLtransferClosingBalanceViewModel: transferClosingBalanceViewModel, gLDetailViewModel: gLDetailViewModelList };
        $.ajax({
            url: "../GLBalanceTransfer/GLBalanceTransfer?AccessMode=" + accessMode + "",
            cache: false,
            type: "POST",
            dataType: "json",
            ...
            success: function (data) {
                if (data.message == "GL Closing Balance Transfer Successfully") {
                    ShowModel("Alert", data.message);

                    setTimeout(
                function () {
                    window.location.href = "../GLBalanceTransfer/ListGLBalanceTransfer";
                }, 2000)
