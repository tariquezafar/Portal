...
function GetFinYearDetail() {
    $.ajax({
        type: "GET",
        asnc: false,
        url: "../BalanceTransfer/GetFinYearDetail",
        data: { },
        dataType: "json",
...
function GetFinYearProducts() {
    var hdnStartDate = $("#hdnStartDate");
    var hdnEndDate = $("#hdnEndDate");
    var ddlCompanyBranch = $("#ddlCompanyBranch");   
    var requestData = { companyBranchId: ddlCompanyBranch.val(), fromDate: hdnStartDate.val(), toDate: hdnEndDate.val() };
    $.ajax({
        url: "../BalanceTransfer/GetFinYearProducts",
        data: requestData,
        dataType: "html",
        type: "GET",
        error: function (err) {
...
function SaveData() {
    if (confirm("Are you sure to Transfer Product Closing Balance.")) {
        var hdnTransferId = $("#hdnTransferId");
        ...
        var productOpeningViewModelList = [];

        $('#tblFinYearProductList tr').each(function (i, row) {
            var $row = $(row);
            var productId = $row.find("#hdnProductId").val();
            var hdnClosingQty = $row.find("#hdnClosingQty").val();
            ...
            if (productId != undefined) {
                ...
                var productOpening = {
                    ProductId: productId,
                    OpeningQty: hdnClosingQty,
                };
                productOpeningViewModelList.push(productOpening);
            }
        });
        ...
        var requestData = { transferClosingBalanceViewModel: transferClosingBalanceViewModel, productOpeningViewModel: productOpeningViewModelList };
        $.ajax({
            url: "../BalanceTransfer/BalanceTransfer?AccessMode=" + accessMode + "",
            cache: false,
            type: "POST",
            dataType: "json",
            ...
            success: function (data) {
                if (data.message == "Product Closing Balance Transfer Successfully") {
                    ShowModel("Alert", data.message);

                    setTimeout(
                function () {
                    window.location.href = "../BalanceTransfer/ListBalanceTransfer";
                }, 2000)
