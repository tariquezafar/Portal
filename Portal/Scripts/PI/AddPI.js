  ...
  .autocomplete("instance")._renderItem = function (ul, item) {
    ...
    $("#txtProductName").autocomplete({
        ...
        select: function (event, ui) {
            ...
            //If Unregistered Vendor then CGST & SGST & IGST should be 0 as per Hari Sir Discussed====
...
.autocomplete("instance")._renderItem = function (ul, item) {
    ...
    $("#txtRefDate,#txtSearchFromDate,#txtSearchToDate,#txtSISearchFromDate,#txtSISearchToDate").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd-M-yy',
        yearRange: '-10:+100',
    ...
    BindCompanyBranchSaleInvoiceList();
    ...
    var piDocuments = [];
    GetPIDocumentList(piDocuments);

    setTimeout(
      function () {
          var siChasis = [];
          GetSIChasisList(siChasis,hdnPIId.val(), 'PI');
      }, 3500);
...
function BindCompanyBranchSaleInvoiceList() {
    $("#ddlSaleInvoiceCompanyBranch").val(0);
    $("#ddlSaleInvoiceCompanyBranch").html("");
    $.ajax({
        type: "GET",
        url: "../DeliveryChallan/GetCompanyBranchList",
        data: {},
        ...
        asnc: false,
        success: function (data) {
            $("#ddlSaleInvoiceCompanyBranch").append($("<option></option>").val(0).html("Select Sale Invoice Company Branch"));
            $.each(data, function (i, item) {
                $("#ddlSaleInvoiceCompanyBranch").append($("<option></option>").val(item.CompanyBranchId).html(item.BranchName));
            });
...
function CalculateTotalCharges() {
    ...
    $("#txtTotalPrice").val((parseFloat(totalPrice) -
...
function GetPIDocumentList(piDocuments) {
    var hdnPIId = $("#hdnPIId");
    var requestData = { piDocuments: piDocuments, piId: hdnPIId.val() };
    $.ajax({
        url: "../PurchaseInvoice/GetPISupportingDocumentList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
...
function SaveDocument() {
    ...
    $.ajax({
        url: "../PurchaseInvoice/SaveSupportingDocument",
        type: "POST",
        asnc: false,
        ...
        success: function (result) {
            if (result.status == "SUCCESS") { 
                ...
                var hdnInvoiceDocId = $("#hdnInvoiceDocId");
                ...
                $('#tblDocumentList tr').each(function (i, row) {
                    ...
                    var invoiceDocId = $row.find("#hdnInvoiceDocId").val();
                    ...
                    if (invoiceDocId != undefined) {
                        if ((hdnDocumentSequence.val() != documentSequenceNo)) {
                            ...
                            piDocumentList.push(piDocument);
                            docEntrySequence = parseInt(docEntrySequence) + 1;
                        }
                        else if (hdnInvoiceDocId.val() == invoiceDocId && hdnDocumentSequence.val() == documentSequenceNo) {
                            var piDocument = {
                                InvoiceDocId: hdnInvoiceDocId.val(),
                ...
                piDocumentList.push(piDocumentAddEdit);
                hdnDocumentSequence.val("0");

                GetPIDocumentList(piDocumentList);
...
function AddTax(action) {
    ...
    $('#tblTaxList tr').each(function (i, row) {
        ...
        if (taxName != undefined) {
            ...
            else if (hdnPITaxDetailId.val() == piTaxDetailId && hdnTaxSequenceNo.val() == taxSequenceNo)
            {
                var piTaxAddEdit = {
...
function GetPIDetail(invoiceId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#hdnSaleInvoiceId").val(data.SaleInvoiceInvoiceId);
            $("#txtSaleInvoiceNo").val(data.SaleInvoiceInvoiceNo);

            if(data.POId!="0")
            {
                $("#txtConsigneeName").attr('readOnly', true);
            ...
            $("#ddlSaleInvoiceCompanyBranch").val(data.SICompanyBranchId);
            ...
            $("#ddlPurchaseType").val(data.PurchaseType);
...
function SaveData() {
    var txtPINo = $("#txtPINo");
    var hdnPIId = $("#hdnPIId");
    var txtPIDate = $("#txtPIDate");
    var hdnPOId = $("#hdnPOId");
    var txtPONo = $("#txtPONo");

    var SaleInvoiceInvoiceId = $("#hdnSaleInvoiceId");
    var SaleInvoiceInvoiceNo= $("#txtSaleInvoiceNo");
    ...
    var txtRemarksValue = $("#txtRemarksValue");
    ...
    var ddlSaleInvoiceCompanyBranch = $("#ddlSaleInvoiceCompanyBranch");
    ...
    var piViewModel = {
        ...
        InvoiceDate: txtPIDate.val().trim(),
        POId:hdnPOId.val().trim(),
        ...
        Remarks: txtRemarksValue.val(),
        RoundOfValue: txtRoundOfValue.val(),
        GrossValue: txtGrossValue.val(),
        PurchaseType: ddlPurchaseType.val(),
        CompanyBranchId: ddlCompanyBranch.val(),
        SICompanyBranchId: ddlSaleInvoiceCompanyBranch.val(),
        SaleInvoiceInvoiceId: SaleInvoiceInvoiceId.val(),
        SaleInvoiceInvoiceNo: SaleInvoiceInvoiceNo.val()
    };
    ...
    $('#tblTaxList tr').each(function (i, row) {
        var $row = $(row);
        var invoiceTaxDetailId = $row.find("#hdnPITaxDetailId").val();
        var taxId = $row.find("#hdnTaxId").val();
        var taxName = $row.find("#hdnTaxName").val();
        var taxPercentage = $row.find("#hdnTaxPercentage").val();
        var taxAmount = $row.find("#hdnTaxAmount").val();
    ...
    $('#tblProductSerialDetail tr').each(function (i, row) {
        ...
        if (productId != undefined) {
            var piChasis = {               
                ProductId: productId,
                ChasisSerialNo: refSerial1
            };
            piChasisList.push(piChasis);
        }
    ...
    //            var $row = $(row);
    //            var productIdAll = $row.find("#hdnProductId").val();
    ...
    if (hdnPIId.val() != null && hdnPIId.val() != 0) {
        accessMode = 2;//Edit Mode
    }
    var requestData = { piViewModel: piViewModel, piProducts: piProductList, piTaxes: piTaxList, piTerms: piTermList, piDocuments: piDocumentList, piChasisLists: piChasisList };
    $.ajax({
        url: "../PurchaseInvoice/AddEditPI?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                  function () {
                      window.location.href = "../PurchaseInvoice/AddEditPI?invoiceId=" + data.trnId + "&AccessMode=3";
                  }, 2000);
...
function SelectPO(poId, poNo, poDate, vendorId, vendorCode, vendorName) {  
    ...
    var poTaxes = [];
    GetPOTaxList(poTaxes, poId);
    var poTerms = [];
    GetPOTermList(poTerms, poId);
...
function GetPOTaxList(poTaxes, poId) {
 
    var requestData = { poTaxes: poTaxes, poId: poId };
    $.ajax({
        url: "../PO/GetPOTaxList",
        cache: false,
        data: JSON.stringify(requestData),
...
function GetPOTermList(poTerms, poId) {
    
    var requestData = { poTerms: poTerms, poId: poId };
    $.ajax({
...
function GetPOProductList(poProducts, poId) {
    ...
    $.ajax({
        url: "../PurchaseInvoice/GetPOProductList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
...
function SearchPO() {
    ...
    var ddlCompanyBranch = $("#ddlCompanyBranch");
    var requestData = {
        poNo: txtSearchPONo.val().trim(), vendorName: txtVendorName.val().trim(), refNo: txtRefNo.val().trim(), fromDate: txtFromDate.val(), toDate: txtToDate.val(), approvalStatus: "Approved", displayType: "Popup",
        CreatedByUserName: txtSearchCreatedBy.val(), companyBranch: ddlCompanyBranch.val()
    };
    $.ajax({
...
function SearchProductPurchase() {
    ...
    var ddlCompanyBranch = $("#ddlCompanyBranch");
    $.ajax({
        url: "../PurchaseInvoice/GetPorductPurchaseList",
...
function OpenSISearchPopup() {
    if ($("#ddlSaleInvoiceCompanyBranch").val() == "0" || $("#ddlSaleInvoiceCompanyBranch").val() == "") {
        ShowModel("Alert", "Please Select Sale Invoice Company Branch");
        return false;
    }
    ...
    if ($("#hdnConsigneeId").val() == "0" || $("#hdnConsigneeId").val() == "") {
        ShowModel("Alert", "Please Select Consignee Name From List.");
        return false;
    }
...
function SearchSI() {
    ...
    var txtSearchConsigneeName = $("#txtSearchConsigneeName");

    var txtSISearchRefNo = $("#txtSISearchRefNo");
    var txtSISearchCreatedBy = $("#txtSISearchCreatedBy");
    var txtFromDate = $("#txtSISearchFromDate");
    var txtToDate = $("#txtSISearchToDate");
    var ddlCompanyBranch = $("#ddlSaleInvoiceCompanyBranch");

    var requestData = {
        invoiceNo: txtSearchSaleInvoiceNo.val().trim(),
        customerName: txtSearchConsigneeName.val().trim(),
        refNo: txtSISearchRefNo.val().trim(),
        fromDate: txtFromDate.val(),
        toDate: txtToDate.val(),
        displayType: "Popup",
        approvalStatus: "Final",

        CreatedByUserName: txtSISearchCreatedBy.val(),
        companyBranch: ddlCompanyBranch.val()
    };
    $.ajax({
        url: "../PurchaseInvoice/GetSIList",
        data: requestData,
        dataType: "html",
        type: "GET",
        error: function (err) {
...
function SelectSI(invoiceId, invoiceNo, invoiceDate) {


    $("#hdnSaleInvoiceId").val(invoiceId);
    $("#txtSaleInvoiceNo").val(invoiceNo);
    $("#txtPIDate").val(invoiceDate);
    ...
    var siChasis = [];
    GetSIChasisList(siChasis,invoiceId, 'SI');


    $("#SearchSIModel").modal('hide');
...
function GetSaleInvoiceProductList(saleinvoiceProducts, invoiceId) {
    ...
    $.ajax({
        url: "../PurchaseInvoice/GetSaleInvoiceProductList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
...
function GetSIChasisList(siChasis,invoiceId, mode) {
    var requestData = {
        siChasis: siChasis,
        invoiceId: invoiceId,
        mode: mode,
    ...
    $.ajax({
        url: "../PurchaseInvoice/GetSIChaisList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
        ...
        error: function (err) {
            $("#divChasisList").html("");
            $("#divChasisList").html(err);
        },
        success: function (data) {
            $("#divChasisList").html("");
            $("#divChasisList").html(data);
        }
    });
}
