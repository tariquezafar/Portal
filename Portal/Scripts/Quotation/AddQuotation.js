    ...
    $("#txtCustomerName").autocomplete({
       ...
       // },
        select: function (event, ui) {
           ...
           // $("#txtCSTNo").val(ui.item.gSTNo);  these value updated below in GetCustomerDetail
...
function SaveDocument() {
    ...
    $.ajax({
        url: "../Quotation/SaveSupportingDocument",
        type: "POST",
        asnc: false,
...
function CalculateTotalCharges() {
    ...
    $("#txtTotalPrice").val(((totalPrice + CGSTAmount + SGSTAmount + IGSTAmount) - discountAmount).toFixed(2));
...
function ReCalculateNetValues() {
    ...
    + parseFloat(freightValue) + parseFloat(freightCGST_Amount) + parseFloat(freightSGST_Amount)
    + parseFloat(freightIGST_Amount) + parseFloat(loadingValue) + parseFloat(loadingCGST_Amount)
    + parseFloat(loadingSGST_Amount) + parseFloat(loadingIGST_Amount) + parseFloat(insuranceValue)
    + parseFloat(insuranceCGST_Amount) + parseFloat(insuranceSGST_Amount) + parseFloat(insuranceIGST_Amount)
    + parseFloat(rTORegValue) + parseFloat(rTORegCGST_Amount) + parseFloat(rTORegSGST_Amount) + parseFloat(rTORegIGST_Amount)
    + parseFloat(vehicleInsurance)).toFixed(0));
}
...
SetGSTPercentageInProduct();
    ...
    SetGSTPercentageInProduct();  // Commented as it is not required.
    setTimeout(
function () {
    SaveData();
    }, 2000);
...
function SaveData() {
    ...
    $('#tblDocumentList tr').each(function (i, row) {
        ...
        if (quotationDocId != undefined) {
            ...
            quotationDocumentList.push(quotationDocument);
        }
    ...
    var requestData = { quotationViewModel: quotationViewModel, quotationProducts: quotationProductList, quotationTaxes: quotationTaxList, quotationTerms: quotationTermList, quotationDocuments: quotationDocumentList };
    $.ajax({
        url: "../Quotation/AddEditQuotation?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../Quotation/AddEditQuotation?QuotationId=" + data.trnId + "&AccessMode=3";
                   }, 2000);
...
function ResetPage() {
    if (confirm("Are you sure to reset the page")) {
        window.location.href = "../Quotation/AddEditQuotation";
    }
}
