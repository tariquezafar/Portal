$(document).ready(function () {
    ...
    $('#txtMaterialRejectNoteNo').attr('readOnly', true);

    $('#txtMaterialRejectNoteDate').attr('readOnly', true);
   

    $("#txtVendorName").attr('readOnly', true);
    $("#txtPoDate").attr('readOnly', true);

    $("#txtSearchFromDatePopup").attr('readOnly', true);
    $("#txtSearchToDatePopup").attr('readOnly', true);
    ...
    $("#txtGateInDate").attr('disabled', true);
    $("#txtQualityCheckDate").attr('disabled', true);
...
.autocomplete("instance")._renderItem = function (ul, item) {
    ...
    $("#txtSearchFromDatePopup,#txtSearchToDatePopup").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd-M-yy',
        yearRange: '-10:+100',
    ...
    $("#txtMaterialRejectNoteDate").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd-M-yy',
        yearRange: '-10:+100',
 ...
 var hdnMaterialReceiveNoteId = $("#hdnMaterialReceiveNoteId");
 if (hdnMaterialReceiveNoteId.val() != "" && hdnMaterialReceiveNoteId.val() != "0" && hdnAccessMode.val() != "" && hdnAccessMode.val() != "0") {
        setTimeout(
       function () {
          
           GetMaterialRejectNoteDetail(hdnMaterialReceiveNoteId.val());
        ...
        if (hdnAccessMode.val() == "3") {
            ...
            $("#txtMaterialRejectNoteDate").attr('disabled', true);
            if ($(".editonly").hide()) {
 ...
 GetQualityCheckRekectProduct(hdnMaterialReceiveNoteId.val());
...
function SaveData() {
    ...
    var hdnMaterialReceiveNoteId=$("#hdnMaterialReceiveNoteId");
    var txtMaterialRejectNoteNo=$("#txtMaterialRejectNoteNo");
    ...
    var txtMaterialRejectNoteDate = $("#txtMaterialRejectNoteDate");

    if (txtMaterialRejectNoteDate.val() < txtQualityCheckDate.val())
    {
        ShowModel("Alert", "MaterialRejectNote Date can't be lesser than Quality Check Date !.");
        txtRejectRemark.focus();
        return false;
    }

    if (txtRejectRemark.val() == "" || txtRejectRemark.val() == "0") {
        ShowModel("Alert", "Please enter the Reject Remarks.")
        txtRejectRemark.focus();
        return false;
    }
  
    var materialRejectNoteViewModel = {
        MaterialReceiveNoteId: hdnMaterialReceiveNoteId.val(),
        MaterialReceiveNoteNo: txtMaterialRejectNoteNo.val().trim(),
        MaterialReceiveNoteDate: txtMaterialRejectNoteDate.val().trim(),

        QualityCheckId: hdnQualityCheckId.val().trim(),
        QualityCheckNo:txtQualityCheckNo.val().trim(),
    ...
    $('#tblProductList tr').each(function (i, row) {
        var $row = $(row);
        var materialRejectNoteProductDetailId = $row.find("#hdnMaterialRejectNoteProductDetailId").val();
        var productId = $row.find("#hdnProductId").val();
        var price = $row.find("#hdnPrice").val();
        var quantity = $row.find("#hdnQuantity").val();
        var receivedQuantity = $row.find("#hdnReceivedQuantity").val();
        ...
        if (productId != undefined) {
            ...
            var MrnProducts = {
                MaterialRejectNoteProductDetailId: materialRejectNoteProductDetailId,
                MaterialReceiveNoteId: 0,
                ProductId: productId,
                Price:price,
                Quantity: quantity,
                ReceivedQuantity: receivedQuantity,
            ...
            mrnProductList.push(MrnProducts);
        }
    });
    ...
    var requestData = { materialRejectNoteViewModel: materialRejectNoteViewModel, mrnProductList: mrnProductList };
    $.ajax({
        url: "../MaterialRejectNote/AddEditMaterialRejectNote?AccessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../MaterialRejectNote/AddEditMaterialRejectNote?MaterialReceiveNoteId=" + data.trnId + "&AccessMode=3";
...
function SearchQC() {
    var txtQualityCheckNo = $("#txtQualityCheckNoPopup");
    var txtSearchGateInNO = $("#txtSearchGateInNOPopup");
    var txtSearchPONO = $("#txtSearchPONOPopup");
    var txtFromDate = $("#txtSearchFromDatePopup");
    var txtToDate = $("#txtSearchToDatePopup");
    var ddlCompanyBranch = $("#ddlCompanyBranch");

    var requestData = {qualityCheckNo: txtQualityCheckNo.val().trim(), gateInNO: txtSearchGateInNO.val().trim(), pONO: txtSearchPONO.val().trim(), fromDate: txtFromDate.val(), toDate: txtToDate.val(), approvalStatus: "Final", displayType: "Popup", companyBranch: ddlCompanyBranch.val() };
    $.ajax({
        url: "../MaterialRejectNote/GetQualityCheckRejectList",
        data: requestData,
        dataType: "html",
        type: "GET",
        error: function (err) {
...
function SelectQualityCheck(qualityCheckId,qualityCheckNo, qualityCheckDate, gateInId, gateInNo, gateInDate, poId, pono, pODate, vendorName,vendorId, companyBranchId) {
    $("#SearchInvoiceModel").modal('hide');

    $("#ddlCompanyBranch").attr('disabled', true);

    $("#hdnQualityCheckId").val(qualityCheckId);

    $("#hdnGateInId").val(gateInId);
    $("#txtGateInNo").val(gateInNo);
    $("#txtGateInDate").val(gateInDate);

    $("#txtQualityCheckNo").val(qualityCheckNo);
    ...
    $("#hdnSessionCompanyBranchId").val(companyBranchId);
    
    GetQualityCheckRekectProduct(qualityCheckId);
}
...
function GetQualityCheckRekectProduct(qualityCheckId) {
  
    var requestData = { qualityCheckId: qualityCheckId };
    $.ajax({
        url: "../MaterialRejectNote/GetQualityCheckProductList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
...
function GetMaterialRejectNoteDetail(materialReceiveNoteId) {
    $.ajax({
        type: "GET",
        asnc: false,
        url: "../MaterialRejectNote/GetMaterialRejectNoteDetail",
        data: { materialReceiveNoteId: materialReceiveNoteId },
        dataType: "json",
        success: function (data) {
            $("#txtMaterialRejectNoteNo").val(data.MaterialReceiveNoteNo);
            $("#hdnMaterialReceiveNoteId").val(data.MaterialReceiveNoteId);
            $("#txtMaterialRejectNoteDate").val(data.MaterialReceiveNoteDate);
            ...
            $("#hdnSessionCompanyBranchId").val(data.CompanyBranchId);
            $("#hdnSessionUserID").val(data.CreatedBy);
...
function ResetPage() {
    if (confirm("Are you sure to reset the page")) {
        window.location.href = "../MaterialRejectNote/AddEditMaterialRejectNote";
    }
}