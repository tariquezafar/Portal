...
function SaveData() {
    ...
    $.ajax({
        url: "../PMS_Assessment/AddEditAppraiserAssessment?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                if (accessMode == 2) {
                    setTimeout(
                  function () {
                      window.location.href = "../PMS_Assessment/ListAppraiserAssessment";
                  }, 2000);
                }
                else {
                    setTimeout(
                    function () {
                        window.location.href = "../PMS_Assessment/AddEditAppraiserAssessment";
                    }, 2000);
                }
            }
            else {
...
function GetEmployeeGoalList(goalList) {
    ...
    $.ajax({
        url: "../PMS_Assessment/GetAppraiserAssessmentGoalList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
