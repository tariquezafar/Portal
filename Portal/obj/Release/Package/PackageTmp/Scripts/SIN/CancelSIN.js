...
function SaveData() {
    ...
    if (txtCancelReason.val().trim() == "" || txtCancelReason.val().trim() == "0") {
    ...
    if (CalculateIssuedQuantity()) {
       ...
       // var requestData = { sinViewModel: sinViewModel, sinProductDetailViewModel: sinProductList, sinDocuments: sinDocumentList };
        var requestData = { sinId: hdnsINId.val(), cancelReason: txtCancelReason.val(), companyBranchId: ddlCompanyBranch.val(), };
        $.ajax({
            url: "../SIN/CancelSIN",
            cache: false,
            type: "POST",
