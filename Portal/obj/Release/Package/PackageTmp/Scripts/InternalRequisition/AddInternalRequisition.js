$(document).ready(function () {
    ...
    $("#ddlRequisitionType").attr('disabled', true);
...
function GetRequisitionProductList(requisitionProducts) {
    ...
    $.ajax({
        url: "../InternalRequisition/GetInternalRequisitionProductList",
        cache: false,
...
function SaveData() {
    ...
    var storeRequisitionProductList = [];
        ...
        $.ajax({
            url: "../InternalRequisition/AddEditInternalRequisition?AccessMode=" + accessMode + "",
            cache: false,
            type: "POST",
            dataType: "json",
            data: JSON.stringify(requestData),
            ...
            success: function (data) {
                if (data.status == "SUCCESS") {
                    if (hdnRequisitionId.val() != 0 && hdnRequisitionId.val() != "") {
                        ShowModel("Alert", "Internal Requisition Updated Successfully");
                    }
                    else {
                        ShowModel("Alert", "Internal Requisition Created Successfully");
                    }
                    //ClearFields();
                    setTimeout(
                       function () {
                           window.location.href = "../InternalRequisition/AddEditInternalRequisition?internalRequisitionId=" + data.trnId + "&AccessMode=3";
                       }, 1000);
...
function GetStoreRequisitionDetail(requisitionId) {
    $.ajax({
        type: "GET",
        asnc: false,
        url: "../InternalRequisition/GetInternalRequisitionDetail",
