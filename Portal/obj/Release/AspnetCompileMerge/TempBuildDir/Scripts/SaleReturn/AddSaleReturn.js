$(document).ready(function () {
    ...
    $("#txtSaleReturnNo").attr('readOnly', true);
    $("#txtSaleReturnDate").attr('readOnly', true);
    ...
    $("#txtApprovalDate").attr('readOnly', true);
    $("#txtCancelDate").attr('readOnly', true);
    ...
    $("#txtCustomerName").autocomplete({
        ...
        source: function (request, response) {
            $.ajax({
                url: "../SaleReturn/GetCustomerAutoCompleteList",
                type: "GET",
                dataType: "json",
...
.autocomplete("instance")._renderItem = function (ul, item) {
    ...
    $("#txtSaleReturnDate,#txtLRDate,#txtDispatchRefDate,#txtRejectedDate,#txtCancelDate,#txtApprovalDate").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd-M-yy',
        yearRange: '-10:+100',
    ...
    var hdnSaleReturnId = $("#hdnSaleReturnId");
    if (hdnSaleReturnId.val() != "" && hdnSaleReturnId.val() != "0" && hdnAccessMode.val() != "" && hdnAccessMode.val() != "0") {
        setTimeout(
       function () {
           $("#ddlCompanyBranch").attr('disabled', true);
           GetSaleReturnDetail(hdnSaleReturnId.val());
           GetSaleReturnChasisSIProductList(hdnSaleReturnId.val(),2);
       }, 3000);
    ...
    GetSaleReturnProductList(saleReturnProducts);
...
function GetBranchStateId() {
    ...
    $.ajax({
        type: "GET",
        url: "../SaleReturn/GetCompanyBranchByStateIdList",
...
function AddProduct(action) {
    ...
    var hdnSaleReturnProductDetailId = $("#hdnSaleReturnProductDetailId");
    ...
    if (txtQuantity.val().trim() == "" || txtQuantity.val().trim() == "0" || parseFloat(txtQuantity.val().trim()) <= 0) {
        ...
        if (parseFloat(txtQuantity.val().trim()) > $("#hdnQuantity").val()) {
            ShowModel("Alert", "Quantity cannot be greater than Actual Quantity")
            txtQuantity.focus();
            return false;
        }
    }
    if (parseFloat(txtQuantity.val().trim()) > parseFloat(txtAvailableStock.val().trim())) {
        ShowModel("Alert", "Sale Return Quantity cannot be greater than Available Stock Quantity")
        txtQuantity.focus();
        return false;
    }
    ...
    $('#tblProductList tr').each(function (i, row) {
        ...
        var saleReturnProductDetailId = $row.find("#hdnSaleReturnProductDetailId").val();
            ...
            if (action == 1 || (hdnSequenceNo.val() != sequenceNo)) {
                ...
                if (productId == hdnProductId.val() && parseFloat(price) != parseFloat(txtPrice.val())) {
                    if (confirm("You have allready Added This Product with Different Price. Do you want to add it?")) {
                        var saleReturnProduct = {
                            SaleReturnProductDetailId: saleReturnProductDetailId,
    ...
    if (action == 1) {
        ...
        saleReturnProductList.push(saleReturnProductAddEdit);
        hdnSequenceNo.val("0");
    }
    GetSaleReturnProductList(saleReturnProductList);

}
function GetSaleReturnProductList(saleReturnProducts) {
    var hdnSaleReturnId = $("#hdnSaleReturnId");
    var requestData = { saleReturnProducts: saleReturnProducts, saleReturnId: hdnSaleReturnId.val() };
    $.ajax({
        url: "../SaleReturn/GetSaleReturnProductList",
        cache: false,
...
function CalculateGrossandNetValues() {
    ...
    $("#txtTotalValue").val(Math.round(parseFloat(parseFloat(basicValue) + parseFloat(taxValue) + parseFloat(freightValue) + parseFloat(freightCGST_Amt) + parseFloat(freightSGST_Amt) + parseFloat(freightIGST_Amt) + parseFloat(loadingValue) + parseFloat(loadingCGST_Amt) + parseFloat(loadingSGST_Amt) + parseFloat(loadingIGST_Amt) + parseFloat(insuranceValue) + parseFloat(insuranceCGST_Amt) + parseFloat(insuranceSGST_Amt) + parseFloat(insuranceIGST_Amt)).toFixed(2)));
...
function EditProductRow(obj) {
    ...
    $("#hdnSaleReturnProductDetailId").val(saleReturnProductDetailId);
...
function GetSaleReturnDetail(saleReturnId) {
    $.ajax({
        type: "GET",
        asnc: false,
        url: "../SaleReturn/GetSaleReturnDetail",
        data: { saleReturnId: saleReturnId },
        dataType: "json",
        success: function (data) {
            $("#txtSaleReturnNo").val(data.SaleReturnNo);
            $("#txtSaleReturnDate").val(data.SaleReturnDate);
            ...
            $("#txtDispatchRefNo").val(data.RefNo);
            $("#txtDispatchRefDate").val(data.RefDate);                       
...
function SaveData() {
    ...
    var hdnSaleReturnId = $("#hdnSaleReturnId");
    var txtSaleReturnDate = $("#txtSaleReturnDate");
    ...
    if (ddlReturnType.val() == "" || ddlReturnType.val() == "0") {
        ShowModel("Alert", "Please select Return Type")
        ddlReturnType.focus();
        return false;
    }
    ...
    if (txtBasicValue.val() == "" || parseInt(txtBasicValue.val()) <= 0) {
    ...
    if (txtLoadingCGST_Amt.val() == "" || parseInt(txtLoadingCGST_Amt.val()) == 0) {
        loadingCGSTPerc = 0;
    }
    else {
    ...
    if (txtLoadingSGST_Amt.val() == "" || parseInt(txtLoadingSGST_Amt.val()) == 0 || txtLoadingSGST_Amt.val()==null) {
        loadingSGSTPerc = 0;
    }
    else {
    ...
    if (txtLoadingIGST_Amt.val() == "" || parseInt(txtLoadingIGST_Amt.val()) == 0 || txtLoadingIGST_Amt.val()==null) {
        loadingIGSTPerc = 0;
    }
    else {
    ...
    if (txtFreightCGST_Amt.val() == "" || parseInt(txtFreightCGST_Amt.val()) == 0 || txtFreightCGST_Amt.val()==null) {
        freightCGSTPerc = 0;
    }
    else {
    ...
    if (txtFreightSGST_Amt.val() == "" || parseInt(txtFreightSGST_Amt.val()) == 0 || txtFreightSGST_Amt.val()==null) {
        freightSGSTPerc = 0;
    }
    else {
    ...
    if (txtFreightIGST_Amt.val() == "" || parseInt(txtFreightIGST_Amt.val()) == 0 || txtFreightIGST_Amt.val()==null) {
        freightIGSTPerc = 0;
    }
    else {
    ...
    if (txtInsuranceCGST_Amt.val() == "" || parseInt(txtInsuranceCGST_Amt.val()) == 0 || txtInsuranceCGST_Amt.val()==null) {
        insuranceCGSTPerc = 0;
    }
    else {
    ...
    if (txtInsuranceSGST_Amt.val() == "" || parseInt(txtInsuranceSGST_Amt.val()) == 0 || txtInsuranceSGST_Amt.val()==null) {
        insuranceSGSTPerc = 0;
    }
    else {
    ...
    if (txtInsuranceIGST_Amt.val() == "" || parseInt(txtInsuranceIGST_Amt.val()) == 0 || txtInsuranceIGST_Amt.val()==null) {
        insuranceIGSTPerc = 0;
    }
    else {
    ...
    var saleReturnViewModel = {
        SaleReturnId: hdnSaleReturnId.val(),
        SaleReturnNo: txtSaleReturnNo.val().trim(),
        SaleReturnDate: txtSaleReturnDate.val().trim(),
        ...
        BillingAddress: txtSAddress.val().trim(),
        City: txtSCity.val().trim(),
        StateId: ddlSState.val(),
        CountryId: ddlSCountry.val(),
        PinCode: txtSPinCode.val().trim(),
        GSTNo: txtSGSTNo.val().trim(),
        Email: txtSEmail.val().trim(),
        MobileNo: txtSMobileNo.val().trim(),
        ContactNo: txtSContactNo.val().trim(),
        Fax: txtSFax.val().trim(),        
        RefNo: txtDispatchRefNo.val().trim(),
        RefDate: txtDispatchRefDate.val(),       
    ...
    ////////////// Start SaleRetrunProductSerialDetailViewModel ///////////

    var SaleRetrunProductSerialDetailViewModel = [];
    $('#tblSaleReturnChasisSIProductList tr').each(function (i, row) {
        var $row = $(row);
        var chkstatus = $row.find("#chkstatus").is(':checked') ? true : false;
        var ProductId = $row.find("#hdnProductId").val();
        var RefSerial1 = $row.find("#hdnRefSerial1").val();
        var InvoiceSerialId = $row.find("#hdnInvoiceSerialId").val();

        if ($("#ddlApprovalStatus").val() == 'Final') {
            if (chkstatus == true) {
                if (ProductId != undefined) {
                  
                    var SaleRetrunProductSerial = {
                        ProductId: ProductId,
                        InvoiceSerialId: InvoiceSerialId,
                        RefSerial1: RefSerial1,
                        serialStatus: chkstatus
                        
                    };
                    SaleRetrunProductSerialDetailViewModel.push(SaleRetrunProductSerial);
                }
            }
        }
        else
    ...
    ////////////// End SaleRetrunProductSerialDetailViewModel ///////////
    ...
    ///Check Serilized Product Quantity ///////////
    ...
    $('#tblProductList tr:not(:has(th))').each(function (i, row) {
        ...
        if (productId != undefined) {
            if (isSerializedProduct.toUpperCase() == "TRUE") {
                $('#tblSaleReturnChasisSIProductList tr:not(:has(th))').each(function (i, row) {
    ...
    ///End Check Serilized Product Quantity ///////////


   
    var accessMode = 1;//Add Mode
    if (hdnSaleReturnId.val() != null && hdnSaleReturnId.val() != 0) {
        accessMode = 2;//Edit Mode
    }

    var requestData = { saleReturnViewModel: saleReturnViewModel, saleReturnProducts: saleReturnProductList, saleRetrunProductSerialDetailViewModel: SaleRetrunProductSerialDetailViewModel };
    $.ajax({
        url: "../SaleReturn/AddEditSaleReturn?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../SaleReturn/AddEditSaleReturn?SaleReturnId=" + data.trnId + "&AccessMode=3";
                   }, 2000);
...
function GetProductAvailableStock(productId) {
    $.ajax({
        ...
        data: { productid: productId, companyBranchId: $("#ddlCompanyBranch").val(), trnId: $("#hdnSaleReturnId").val(), trnType: "DC" },
        dataType: "json",
        success: function (data) {
...
function SearchInvoice() {
    ...
    $.ajax({
        url: "../SaleReturn/GetSaleReturnSIList",
        data: requestData,
        dataType: "html",
        type: "GET",
        error: function (err) {
...
function SelectInvoice(invoiceId, saleinvoiceNo, invoiceDate, customerId, customerCode, customerName, consigneeId, consigneeCode, consigneeName) {
  ...
  //  GetSaleReturnChasisSIProductList(invoiceId);
    ...
    GetSaleReturnChasisSIProductList(invoiceId,1);
    
}

function GetSaleReturnChasisSIProductList(invoiceId,mode) {
   
    var requestData = { saleinvoiceId: invoiceId, mode: mode };
    $.ajax({
        url: "../SaleReturn/GetSaleReturnChasisSIProductList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
        type: "POST",
        error: function (err) {
            $("#divChasisSerialProductList").html("");
            $("#divChasisSerialProductList").html(err);
        },
        success: function (data) {
            $("#divChasisSerialProductList").html("");
            $("#divChasisSerialProductList").html(data);
         }
    });
}
...
function GetSaleInvoiceProductList(saleinvoiceProducts, invoiceId) {
    ...
    $.ajax({
        url: "../SaleReturn/GetSaleReturnSIProductList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
...
if (confirm("Are you sure to reset the page")) {
    window.location.href = "../SaleReturn/AddEditSaleReturn";
}
}
