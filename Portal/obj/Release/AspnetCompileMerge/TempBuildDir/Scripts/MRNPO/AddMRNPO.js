...
function BindCompanyBranchList1() {
    $("#ddlCompanyBranch1").val(0);
    $("#ddlCompanyBranch1").html("");
    $.ajax({
        type: "GET",
        ...
        asnc: false,
        success: function (data) {
            $("#ddlCompanyBranch1").append($("<option></option>").val(0).html("-Select Company Branch-"));
            $.each(data, function (i, item) {
                $("#ddlCompanyBranch1").append($("<option></option>").val(item.CompanyBranchId).html(item.BranchName));
            });
            var hdnSessionCompanyBranchId = $("#hdnSessionCompanyBranchId1");
            var hdnSessionUserID = $("#hdnSessionUserID1");
            if (hdnSessionCompanyBranchId.val() != "0" && hdnSessionUserID.val() != "2") {
                $("#ddlCompanyBranch1").val(hdnSessionCompanyBranchId.val());
                $("#ddlCompanyBranch1").attr('disabled', true);
            }
        },
...
function SaveData() {
    ...
    if (txtPONo.val().trim() == "") {
        ShowModel("Alert", "Please Select PO No.")
        txtPONo.focus();
        return false;
    }
    if (hdnPOId.val() == "" || hdnPOId.val() == "0") {
        ShowModel("Alert", "Please Select PO No.")
        txtPONo.focus();
        return false;
    }

    var pDate = new Date(txtPODate.val());
    var mDate = new Date(txtMRNDate.val());
    if (mDate < pDate) {
        ShowModel("Alert", "MRN Date not be Less than PO Date.")
        return false;
    }
    ...
    if (productSelected == false) {
        ShowModel("Alert", "Please select at least one Product Received  Quantity.")
        return false; 
    }

    var requestData = { mrnViewModel: MRNViewModel, mrnProducts: mrnProductList, mrnDocuments: mrnDocumentList };
    $.ajax({
        url: "../MRNPO/AddEditMRNPO?AccessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../MRNPO/AddEditMRNPO?mrnId=" + data.trnId + "&AccessMode=3";
                   }, 2000);
...
function SearchPO() {
    ...
    var ddlCompanyBranch1 = $("#ddlCompanyBranch");

    var requestData = { poNo: txtSearchPONo.val().trim(), vendorName: txtVendorName.val().trim(), refNo: txtRefNo.val().trim(), fromDate: txtFromDate.val(), toDate: txtToDate.val(), approvalStatus: "Final", displayType: "Popup",companyBranch: ddlCompanyBranch1.val() };
    $.ajax({
        url: "../MRNPO/GetPOList",
        data: requestData,
        dataType: "html",
...
function SelectPO(poId, pono, poDate, vendorId, vendorCode, vendorName) {
    ...
    $("#txtMRNDate").datepicker("option", "minDate", poDate);
    GetPODetail(poId);
    var poProducts = [];
...
function ResetPage() {
    if (confirm("Are you sure to reset the page")) {
        window.location.href = "../MRNPO/AddEditMRNPO";
    }
}
