...
function SaveData() {
    ...
    $.ajax({
        url: "../PMS_Assessment/AddEditEmployeeSelfAssessment?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                if (accessMode == 2) {
                    setTimeout(
                  function () {
                      window.location.href = "../PMS_Assessment/AddEditEmployeeSelfAssessment";
                  }, 2000);
                }
                else {
                    setTimeout(
...
function GetEmployeeGoalList(goalList) {
    ...
    $.ajax({
        url: "../PMS_Assessment/GetEmployeeSelfAssessmentGoalList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
...
function AddGoal(action) {
    ...
    if ((txtSelfScore.val().trim() <= 0) || (txtSelfScore.val().trim() > 19)) {
        ShowModel("Alert", "Please Enter Correct Self Score")
        return false;
    }
