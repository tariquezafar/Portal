...
function SaveData() {
    ...
    var hdnAssemblingProcessId = $("#hdnAssemblingProcessId");
    var txtAssemblingProcessDate = $("#txtAssemblingProcessDate");
    ...
    var ddlAssemblingProcessStatus = $("#ddlAssemblingProcessStatus");
    var txtRemarks1 = $("#txtRemarks1");
    var txtRemarks2 = $("#txtRemarks2");
    

    if (hdnPaintProcessId.val() == "" || ddlCompanyBranch.val() == 0) {
        ShowModel("Alert", "Please select  Paint Process No.")
        txtPaintProcessNo.focus();
        return false;
    }
    ...
    var assemblingProcessViewModel = {
        ...
        AssemblingProcessStatus: ddlAssemblingProcessStatus.val(),
        Remarks1: txtRemarks1.val(),
        Remarks2: txtRemarks2.val()
    };

    var assemblingProcessChasisSerialList = [];
    $('#tblPrintChasis tr').each(function (i, row) {
        var $row = $(row);
        var productId = $row.find("#hdnProductID").val();
        ...
        if (productId != undefined) {           
            if (chkPrintChasis == true) {
                var assemblingProcessChasisSerial = {
                    ProductId: productId,
                    ChasisSerialNo: chasisSerialNo.trim(),
                    MotorNo: motorNo.trim()
                };
                assemblingProcessChasisSerialList.push(assemblingProcessChasisSerial);
            }          
        }
    });
    var totalQuantity = 0;
    ...
    $('#tblProductList tr').each(function (i, row) {
        ...
        if (productId != undefined) {
            ...
            if (ddlAssemblingProcessStatus.val() == "Final") {
                if (parseFloat(quantity) > 0) {
                    var assemblingProcessProduct = {
                        AssemblingProcessDetailId: assemblingProcessDetailId,
                        ProductId: productId,
    ...
    if (assemblingProcessProductList.length == 0) {
    ...
    if (checkedFlag == false) {
       ...
       var flag = true;
        ...
        if (hdnAssemblingProcessId.val() != null && hdnAssemblingProcessId.val() != 0) {
    ...
    var requestData = { assemblingProcessViewModel: assemblingProcessViewModel, assemblingProcessProducts: assemblingProcessProductList, assemblingProcessChasisSerialProducts: assemblingProcessChasisSerialList };
    $.ajax({
        url: "../AssemblingProcess/AddEditAssemblingProcess?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../AssemblingProcess/AddEditAssemblingProcess?assemblingProcessId=" + data.trnId + "&AccessMode=3";
                   }, 2000);
