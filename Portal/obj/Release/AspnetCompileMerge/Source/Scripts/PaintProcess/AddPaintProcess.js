...
function AddProduct(action) {
    ...
    var hdnWorkOrderQTY = $("#hdnWorkOrderQTY");

    

    var txtAdjQTY = $("#txtAdjQTY");
    ...
    if (hdnWorkOrderQuantity.val() == "" || hdnWorkOrderQuantity.val() == 0)
    {
        wQty = parseFloat(hdnWorkOrderQTY.val());
    }
    else
    {
        wQty = (parseFloat(hdnWorkOrderQuantity.val()) + parseFloat(hdnWorkOrderQTY.val()));
    }
    ...
    if (parseFloat(hdnWorkOrderQuantity.val()) > parseFloat(txtAdjQTY.val())) {
            ShowModel("Alert", "Work Order Quantity cannot be greater than Work Order Quantity")
            hdnWorkOrderQuantity.focus();
            return false;
        }
            ...
            if (action == 1 || (hdnSequenceNo.val() != sequenceNo)) {
                ...
                if (ddlAdjustment.val() == productId) {
                    var paintProcessProduct = {
                        ...
                        UOMName: uomName,
                        Quantity: quantity,
                        //PendingQuantity: pendingQuantity - (hdnWorkOrderQuantity.val()),
                        PendingQuantity: workOrderQuantity - (hdnWorkOrderQuantity.val()),
                        TotalPaintQuantity: totalRecivedFabQuantity,
                         RecivedQuantity: quantity, 

                       // RecivedQuantity: workOrderQuantity - (hdnWorkOrderQuantity.val()),
                        WorkOrderQuantity: workOrderQuantity - (hdnWorkOrderQuantity.val()),
                ...
                var paintProcessProduct = {
                    ...
                    PendingQuantity: wQty - (txtQuantity.val()),
                    WorkOrderQuantity: wQty,
...
function EditProductRow(obj) {
   // $("#txtWOQTY").attr('readOnly', true);
    ...
    $("#hdnWorkOrderQTY").val(workOrderquantity);
...
function SaveData() {
    var txtPaintProcessNo = $("#txtPaintProcessNo");
    var hdnPaintProcessId = $("#hdnPaintProcessId");
    var txtPaintProcessDate = $("#txtPaintProcessDate");
    var txtWorkOrderNo = $("#txtWorkOrderNo");
    var hdnWorkOrderId = $("#hdnWorkOrderId");    
    var ddlCompanyBranch = $("#ddlCompanyBranch");
    var ddlPaintProcessStatus = $("#ddlPaintProcessStatus");
    ...
    var paintProcessViewModel = {
        PaintProcessId: hdnPaintProcessId.val(),
        PaintProcessNo: txtPaintProcessNo.val().trim(),
        PaintProcessDate: txtPaintProcessDate.val().trim(),
        WorkOrderId: hdnWorkOrderId.val(),
        WorkOrderNo: txtWorkOrderNo.val().trim(),              
        CompanyBranchId: ddlCompanyBranch.val().trim(),
        PaintProcessStatus: ddlPaintProcessStatus.val(),
    ...
    var paintProcessChasisSerialList = [];
    $('#tblPrintChasis tr').each(function (i, row) {
        var $row = $(row);      
        var productId = $row.find("#hdnProductID").val();
        ...
        if (productId != undefined) {           
                if (chkPrintChasis == true) {
                    var paintProcessChasisSerial = {
                        ProductId: productId,
                        ChasisSerialNo: chasisSerialNo.trim(),
                        MotorNo: motorNo.trim()
                    };
                    paintProcessChasisSerialList.push(paintProcessChasisSerial);
                }          
        }
    });

    var totalWoQuantity = 0;
    var totalRecivedQuantity = 0;
    ...
    $('#tblProductList tr').each(function (i, row) {
        ...
        var workOrderQuantity = $row.find("#hdnWorkOrderQuantity").val();
        var quantity = $row.find("#hdnRecivedQuantity").val();
        var newProduct = $row.find("#hdnNewProduct").val();
        if (paintProcessDetailId != undefined) {

            if (parseFloat(quantity) > 0)
            {
                totalRecivedQuantity += parseFloat(quantity);
                checkedFlag = true;
            }
            if (parseFloat(workOrderQuantity) > 0) {
                totalWoQuantity += parseFloat(workOrderQuantity);
            }
            if (ddlPaintProcessStatus.val() == "Final")
            {
                if (parseFloat(quantity) > 0) {
                    var paintProcessProduct = {
                        PaintProcessDetailId: paintProcessDetailId,
    ...
    if (paintProcessProductList.length == 0) {
        ShowModel("Alert", "Please Select at least 1 Product.")
        return false;
    }
    if (parseFloat(totalRecivedQuantity) > parseFloat(totalWoQuantity)) {
        ShowModel("Alert", "Received Quantity cannot be grater than Work Order Quantity.")
        return false;
    }
    ...
    var requestData = { paintProcessViewModel: paintProcessViewModel, paintProcessProducts: paintProcessProductList, paintProcessChasisSerialProducts: paintProcessChasisSerialList };
    $.ajax({
        url: "../PaintProcess/AddEditPaintProcess?accessMode=" + accessMode + "",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../PaintProcess/AddEditPaintProcess?paintProcessId=" + data.trnId + "&AccessMode=3";
                   }, 2000);
...
function ClearFields() {
    ...
    $("#txtPaintProcessNo").val("");
    $("#hdnPaintProcessId").val("0");
    $("#txtWorkOrderNo").val("");
    $("#hdnWorkOrderId").val("0");
    $("#txtPaintProcessDate").val($("#hdnCurrentDate").val());
    $("#ddlPaintProcessStatus").val("Final");
        ...
        $('#tblProductList tr').each(function (i, row) {
            ...
            if (productId != undefined) {                
                if (ddlAdjustment == productId) {
                    $("#txtAdjQTY").val(parseFloat(workOrderQuantity) - parseFloat(recivedQuantity));
                }
            }
        });
    }
